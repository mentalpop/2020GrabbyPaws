...
Shader "Dark Comic Master (Transparent)"
{
   Properties
   {
	...
	_TRANSPARENCYFOLDOUT ("# Transparency", Int) = 0
		_TransparencyNote ("!NOTE Color tint Alpha acts as multiplier on base color alpha", Int) = 0
		[Toggle] _PreserveSpec ("Preserve Specular", Int) = 1
		[Toggle] _Refraction ("Refraction", Int) = 0
		_RefractionNote ("!NOTE Refraction requires Opaque Texture to be enabled [_Refraction == 1]", Int) = 0
		_RefractionStr ("-Refraction Strength [_Refraction == 1]", Range(0,1)) = 0.1
		[Toggle] _RefractionTint ("-Tinted By Base Color [_Refraction == 1]", Int) = 0
		_RefractionEdge ("Blend With Reflection Probe", Range(0,1)) = 0.5
		[Toggle] _OpaqueLines ("Line Art Always Opaque", Int) = 0
	_ENDFOLDOUT ("#", Int) = 0
